...
export default class Reseller {
      ...
      if (Types.Types.TRoles.ADMIN === role) {
        ...
        resellers = resellerModels.map(resellerModel => {
          return Types.Classes.CUser.init(
            ...
            undefined,
            resellerModel.id,
            resellerModel.createdAt.getTime()
          )
        })
      } else {
  ...
  async getContracts(identity: Types.Classes.CUser, timestamp: number) {
    try {
      ...
      const error = new Utils.iKomidaError(
        Utils.iKomidaError.IKOMIDA_RESELLER_SERVICE_GET_RESTAURANTS_EXCEPTION,
        exception?.message
      )
      return error.logAndReturn(this.logger)
    }
  ...
  async lastRevune(identity: Types.Classes.CUser) {
    try {
      ...
      const referral = await currentUserModel.$get('referral')
      const referralRevuneModels = (await referral?.$get('referralRevunes')) ?? []
      const totalRevune = Number(referralRevuneModels?.[0]?.revune ?? 0) + Number(referralRevuneModels?.[0]?.bonus ?? 0)
